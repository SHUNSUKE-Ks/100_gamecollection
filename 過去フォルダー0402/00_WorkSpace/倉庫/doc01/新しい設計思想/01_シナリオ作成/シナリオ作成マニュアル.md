...
### 7.1 episodes.json との関係

Collection画面の「ストーリー」タブから参照される `episodes.json` との連携：

```
src/data/collection/episodes.json  →  src/assets/data/scenarios/scenario.json
         ↓                                        ↓
    startStoryID: "M_01_01_01"     ←→     storyID: "M_01_01_01"
```

| episodes.json | scenario.json | 説明 |
|---------------|---------------|------|
| `episode.chapters[].events[].startStoryID` | 最初のノードの `storyID` | 一致させる |
...
// episodes.json
{
  "id": "event-1-1-1",
  "title": "塔への招待",
  "startStoryID": "M_01_01_01"  // ← ここ
}

// scenario.json
{
  "storyID": "M_01_01_01",  // ← 一致
  "scene": 1,
  "type": "SCENE_START",
  ...
}
...
### 7.2 events.json との関係

クエスト/サブイベント用の `events.json` との連携：

| events.json | quest_XXX.json | 説明 |
|-------------|----------------|------|
| `event.startStoryID` | 最初のノードの `storyID` | 一致させる |
| `startStoryID: null` | - | UI上「準備中」と表示 |
...
// events.json
{
  "id": "quest_003",
  "title": "森の遺跡の調査",
  "startStoryID": "Q_03_01_01"  // ← ここ
}

// quest_003.json
[
  {
    "storyID": "Q_03_01_01",  // ← 一致
    "scene": 1,
    "type": "SCENE_START",
    ...
  }
...
`BATTLE_START` イベント発生後の復帰処理：

```
BATTLE_START ノード (storyID: "Q_03_01_05")
        ↓
    バトル画面遷移
        ↓
    ┌───────────┬───────────┐
...
| 結果 | 復帰先 |
|------|--------|
| 勝利 | `returnFromBattle(storyID)` で次ノードへ |
| 敗北 | リトライ選択画面 or ゲームオーバー |
